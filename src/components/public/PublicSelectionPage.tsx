...
import { Client, ModelPhoto, AgencyPackage } from '../../types';
import { fetchPublicSelectionData, submitPublicSelectionData, getAgencyPackages } from '../../utils/storage';

interface PublicSelectionPageProps {
  token: string;
}

export const PublicSelectionPage: React.FC<PublicSelectionPageProps> = ({ token }) => {
  const [client, setClient] = useState<Client | null>(null);
  const [clientPhotos, setClientPhotos] = useState<ModelPhoto[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true);
      const data = await fetchPublicSelectionData(token);
      if (isMounted) {
        if (data && data.client) {
          setClient(data.client);
          setClientPhotos(data.modelPhotos || []);
          setPackages(data.packages && data.packages.length > 0 ? data.packages : getAgencyPackages());
          setSelectedPhotoIds(data.client.chosenPhotoIds || []);
          if (
            data.client.status === 'Selecionado' ||
            data.client.status === 'Em produção' ||
            data.client.status === 'Entregue'
          ) {
            setIsSubmitted(true);
          }
        }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-white">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm font-medium text-zinc-400">Carregando seu ensaio exclusivo...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-center space-y-4 shadow-2xl">
          ...
          <h1 className="text-xl font-bold">Link de Seleção Não Encontrado</h1>
          <p className="text-xs text-zinc-400 leading-relaxed">
            O link de seleção informado é inválido ou expirou. Por favor, entre em contato com seu fotógrafo para obter um novo link de acesso.
          </p>
        </div>
      </div>
    );
  ...
  const handleConfirmSelection = async () => {
    if (selectedPhotoIds.length === 0) {
      alert('Por favor, selecione ao menos 1 foto favorita antes de confirmar.');
      return;
    }

    try {
      setIsSubmitting(true);
      const updated = await submitPublicSelectionData(token, selectedPhotoIds);
      if (updated) {
        setClient(updated);
        setIsSubmitted(true);
      }
  ...
  const whatsappMessage = encodeURIComponent(
    `Olá! Estava na página de seleção de fotos (${client.name} - ${client.contractedSession}) e gostaria de saber mais sobre os serviços de marketing e os pacotes mensais da Agência!`
  );
  const whatsappUrl = `https://wa.me/5571992955846?text=${whatsappMessage}`;

  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-amber-500 selection:text-black pb-16">
      ...
      <header className="sticky top-0 z-30 bg-zinc-900/90 backdrop-blur-md border-b border-zinc-800/80 px-4 sm:px-8 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold tracking-tight text-white">
              StudioPhoto • Seleção do Cliente
            </h2>
        ...
        <div className="flex items-center gap-2 bg-zinc-800/90 border border-zinc-700/70 px-3.5 py-1.5 rounded-full text-xs font-semibold">
          ...
          <span>{selectedPhotoIds.length} selecionadas</span>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-zinc-900 via-zinc-900/90 to-zinc-950 border border-zinc-800/80 p-6 sm:p-10 text-center space-y-3">
          ...
          <p className="text-xs sm:text-base text-zinc-400 max-w-2xl mx-auto leading-relaxed">
            Selecione abaixo as suas fotos modelo favoritas para o seu{' '}
            <strong className="text-zinc-200 font-semibold">{client.contractedSession}</strong>.
            Clique nas imagens que mais gostar para marcá-las e clique no botão ao final para confirmar o envio da sua seleção.
          </p>

          {isSubmitted && (
            <div className="p-4 bg-emerald-950/60 border border-emerald-800 rounded-2xl max-w-lg mx-auto flex items-center gap-3 text-left">
              ...
              <div>
                ...
                <p className="text-[11px] text-emerald-400/90">
                  Suas {selectedPhotoIds.length} fotos escolhidas já foram encaminhadas para o fotógrafo iniciar a produção.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
              ...
              <span className="text-xs text-zinc-400">
                Escolha a quantidade ideal de fotos para a sua presença e autoridade
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {packages.map((pkg) => {
                const isMatch = selectedPhotoIds.length === pkg.photoCount;
                return (
                  <div
                    key={pkg.id}
        ...
        {/* ------------------------------------------------------------- */}
        {/* MEIO: GALERIA DE FOTOS PARA SELEÇÃO DO CLIENTE                */}
        {/* ------------------------------------------------------------- */}
        <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
              <Camera className="w-4 h-4 text-amber-500" />
              <span>Fotos disponíveis para sua escolha ({clientPhotos.length})</span>
            </h3>
          ...
          {clientPhotos.length === 0 ? (
            <div className="text-center py-16 bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
              <p className="text-xs text-zinc-400">
                Nenhuma foto cadastrada para este ensaio ainda.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
              {clientPhotos.map((photo, index) => {
                ...
                return (
                  <div
                    key={photo.id}
                    className={`group relative cursor-pointer rounded-2xl overflow-hidden border-2 transition-all duration-200 shadow-md select-none touch-manipulation ${
                      isSelected
                  ...
                  >
                    {/* Photo Container Proporção 3:4 Perfeita Vertical */}
                    <div
                      onClick={() => toggleSelectPhoto(photo.id)}
                      className="relative aspect-[3/4] bg-zinc-950 overflow-hidden"
                    >
                      <img
                        src={photo.imageUrl}
                        alt={photo.name}
        ...
        {/* Lightbox Modal de Alta Resolução para o Cliente Inspecionar a Foto */}
        {previewPhoto && (
          <div
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-zinc-800/80 bg-zinc-900/70">
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] sm:text-xs text-amber-400 font-medium truncate block">
                    Visualização em Alta Definição • Proporção 3:4
                  </span>
                </div>
                <button
                  type="button"
              ...
              {/* Photo Display */}
              <div className="p-3 sm:p-6 flex-1 overflow-auto flex items-center justify-center bg-black/60">
                <img
              ...
              <div className="p-3 sm:p-5 border-t border-zinc-800/80 bg-zinc-900/90 flex items-center justify-between gap-3">
                <p className="text-xs text-zinc-400 hidden sm:block">
                  Clique no botão para incluir ou remover esta foto da sua seleção final.
                </p>
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  <button
                    ...
                    className={`w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all cursor-pointer ${
                      selectedPhotoIds.includes(previewPhoto.id)
        ...
        {/* Confirmation Action Floating/Fixed Bar */}
        {clientPhotos.length > 0 && (
          <div className="fixed bottom-4 inset-x-3 sm:sticky sm:bottom-6 z-40 max-w-md mx-auto p-3.5 sm:p-4 bg-zinc-900/95 backdrop-blur-md border border-zinc-700/80 rounded-2xl shadow-2xl space-y-2.5 sm:space-y-3 text-center">
            <div className="flex items-center justify-between text-xs px-1">
              <span className="text-zinc-400">Total Escolhidas:</span>
              <span className="font-bold text-amber-400 text-sm">
                {selectedPhotoIds.length} de {clientPhotos.length} fotos
              </span>
            </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span>Salvando sua escolha...</span>
                </>
              ) : isSubmitted ? (
                <>
                  <CheckCircle2 className="w-4 h-4" />
                  <span>Atualizar Minha Escolha ({selectedPhotoIds.length})</span>
                </>
              ) : (
                <>
                  <Send className="w-4 h-4" />
                  <span>Confirmar Envio da Seleção ({selectedPhotoIds.length})</span>
                </>
              )}
            </button>
          </div>
